...
def readcut(file_name: str, data_name: str = '', frequencies: List[float] = []) -> xr.DataArray:  # FIX ICUT DICTIONARY
    """
    ...
    attributes, cut_type = utils.check_grid_or_cut_type(icomp, ncomp, icut, "cut")
    ...
    # define frequencies and compare the nr. of frequencies with given frequency information (in from .tor file)
    frequency_placeholders = np.arange(0, no_of_frequencies, dtype=float)
    if frequencies:
        if len(frequencies) != len(frequency_placeholders):
            print('Caution! Frequency indications are inconsistent with the computed nr. of frequencies!')
            print('Remark: If frequencies are taken from .tor file, maybe save GRASP project and retry \
                  (unlike the .cut file, the .tor is exported only when saving)!')
            raise
        else:
            frequencies = frequencies
    else: 
        frequencies = frequency_placeholders
