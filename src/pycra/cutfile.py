...
from .utils import *
from .utils_tor import read_tor, read_frequencies
...
def cut(file_names: List[str], data_name: str = None) -> xr.DataArray:
...
def readcut(file_name: str, data_name: str = '', file_name_tor: str = '') -> xr.DataArray:  # FIX ICUT DICTIONARY
    """
    ...
    # define frequencies
    frequencies = np.arange(0, no_of_frequencies, dtype=float)

    # compare nr. of frequencies with information in .tor file
    if file_name_tor:
        dict_graspobj = read_tor(file_name_tor)
        frequencies_torfile, _ = read_frequencies(dict_graspobj, prefix='single_')
        if len(frequencies_torfile) != len(frequencies):
            print('Caution! Frequency indications in .tor file are inconsistent with the computed nr. of frequencies!')
            print('.cut file: %s' % file_name)
            print('.tor file: %s' % file_name_tor)
            print('Suggestion: save GRASP project and retry (unlike the .cut file, the .tor is exported only when saving)!')
            raise
        else: 
            frequencies = frequencies_torfile
    ...
    for frequency_index,_ in enumerate(frequencies):
        for cut_index,_ in enumerate(cut_orientation):
